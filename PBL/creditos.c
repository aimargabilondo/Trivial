...
void showCreditos(SDL_Renderer* renderer, SDL_Window* window) { // Menu sin terminar SOLO FUNCIONA BOTON CERRAR 

    SDL_RenderClear(renderer);

    SDL_Texture* backgroundTexture = loadTexture(renderer, "foto/fondos/creditos.bmp");

    SDL_RenderPresent(renderer);

    const int buttonCount = 1;  // Aumenta el contador de botones a 3
    Button buttons[BUTTON_COUNT];
    buttons[0] = createButton(30 * 30, 4 * 30, renderer, "foto/botones/itxi.bmp", 5);
